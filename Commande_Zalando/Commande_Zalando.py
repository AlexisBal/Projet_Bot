...
def URLGen():
    ...
    model = input("Entrer le modèle du produit (AIR FORCE 1 ’07 AN20  - Baskets basses) :")
    model = model.lower().replace("’", "").replace("  ", "").replace(" - ", "-").replace(" ", "-").replace("é",
                                                                                                           "e").replace(
        "regular", "").replace("--", "-")
...
def scanner(URLs_taille):
    while True:
        ...
        requette_1 = requests.get(URLs_taille[1], headers=header, verify=False, allow_redirects=False)
        time.sleep(1)
        if requette_1.status_code == 200:
            url_produit = URLs_taille[1]
            print("url du produit :", url_produit)
            return url_produit

        requette_2 = requests.get(URLs_taille[0], headers=header, verify=False, allow_redirects=False)
        time.sleep(1)
